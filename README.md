...
**一只正侧面的企鹅**（造型语言按实拍与参考矢量图复刻），**四个品种可切**：

| 品种 | 特征 |
|---|---|
| **巴布亚企鹅**（默认） | 眼睛后上方一块椭圆白斑（手工微调到 `cx 56.8 / cy 17.3 / rx 4.4 / ry 7.9 / -29°`）、红色长喙（喙脊黑线 + 喙尖黑钩 + 喙基黑带）、橘色三趾蹼足带**黑爪** |
| **阿德利企鹅** | 低平的全黑头顶、**细白眼圈**、短而窄的喙、纤细直立的躯干、更窄的白胸、修长的下垂鳍肢、暗粉灰蹼足、向后拖出的长硬尾 |
| **帽带企鹅** | 黑色圆头 + **白脸白喉** + 下颏一道**黑色窄帽带**、短薄近黑的喙（只在口缝留暗棕）、粉灰蹼足；体型介于阿德利与巴布亚之间 |
| **帝企鹅** | 近黑炭灰外套、颈侧一块**黄色泪滴状耳斑**、**淡奶黄上胸渐入白腹**、细长的暗喙、灰黑蹼足；体型最大 |

在 **设置 → 企鹅乐园 → 品种** 里切换，立刻生效；姿态、走路、拉屎、游泳全部照旧，只换皮肤。
每个品种有自己的**舞台尺寸**（按真实体型：帽带 65×74、帝企鹅 83×119、其余 68×82），
所以换个品种连起来看大小关系也是对的。
...
- **新增帽带企鹅（Chinstrap）**：黑色圆头 + **白脸白喉** + 下颏一道**黑色窄帽带**，
  短薄近黑的喙（只在口缝留一道暗棕）、粉灰蹼足、长硬尾；体型介于阿德利与巴布亚之间
  （侧头是重组的：底层只画黑头，白脸层和帽带叠在上面，喙单独画在前景）
- **按品种的体型**：新增 `SPECIES_BODY_SCALE` / `SPECIES_TAIL_OFFSET` / `SPECIES_TAIL_SCALE`
  三张表，并给每个品种配了**舞台尺寸**（帽带 65×74、帝企鹅 83×119、其余 68×82），
  所以四种企鹅放在一起时的大小关系也是对的
- **尾根桥接羽**：阿德利与帽带各有一条固定在躯干上的尾根羽（`*-tail-root-bridge`），
  独立的尾巴旋转时仍与臀部连续，不会露出缝隙
- **精修**：阿德利头顶更平、喙更短更窄、白喉与白腹改成一条连续轮廓（不再有胸前凸角）、
  尾根向左上回收；帽带的眼睛比巴布亚更小、虹膜压暗；各品种虹膜与脚色继续贴近参考个体

### 1.5.0

- **新增两个品种**，设置 → 企鹅乐园 → **品种** 一键切换（造型、动画、拉屎、游泳全部照旧，只换皮肤）：
  - **阿德利企鹅（Adélie）**：低平的全黑头顶、**细白眼圈**、短而暗的喙、纤细直立的躯干、
    更窄的白胸、修长的下垂鳍肢、暗粉灰的蹼足、向后拖出的长硬尾
  - **帝企鹅（Emperor）**：近黑炭灰外套、颈侧一块**黄色泪滴状耳斑**、**淡奶黄上胸渐入白腹**、
    细长的暗喙、灰黑蹼足
- 实现方式是**皮肤替换**而不是重画：造型本来是「描摹参考图 → 拆成 `<use>` 部件」的结构，
  换品种只需对 `PENGUIN_INNER` 做定向替换（头斑填色、喙色与长度、脚色、耳斑/胸口补渐变、
  躯干与鳍肢换成各品种自己的轮廓、尾羽长度系数），离线渲染与浏览器跑同一段字符串，因此可离线验证
- **造型细节迭代**（三个品种共用）：
  - **鳍肢重做**：自然下垂形态 + 肩背之间的宽连接口 + 沿外露轮廓的细白边 + 内侧白面；
    另加一条从翼根连续包到臀部的**后背黑色覆羽**，避免白腹把身体与翅膀视觉切断
  - **眼睛放大**（`pg-side-eye-scale`）
  - **叫声改成实心三角放射线**（参考图里是三角，不是圆形声波）
